...
import math
import sys
import os # Dodane do obsługi folderów
...
def main():
    ...
    problem_name = "drone_problem"
    # Open output file
    with open("domains/p2/problem", 'w') as f:
        # Write the initial part of the problem
        f.write("(defproblem problem p2\n")
        f.write("  (\n")
        ...
        for x in range(len(content_types)):
            ...
            quantity = len(crates_with_contents[x])
            f.write(f"\t(boxes-at {content_name} depot {quantity})\n")

        for i in range(options.carriers):
            carrier = f"carrier{i+1}"
            capacity = random.choice([5, 10, 20])
            f.write(f"\t(transporter-at {carrier} depot)\n")
            f.write(f"\t(transporter-capacity {carrier} {capacity})\n")
            f.write(f"\t(transporter-total-load {carrier} 0)\n")
            for content_name in content_types:
                f.write(f"\t(transporter-content {carrier} {content_name} 0)\n")

        people_locations = [loc for loc in location if loc != "depot"]
        person_to_loc = {p: random.choice(people_locations) for p in person}
        ...
        for loc in people_locations:
            for y in range(len(content_types)):
                ...
                total_need = 0
                for x in range(options.persons):
                    if need[x][y] and person_to_loc[person[x]] == loc:
                        total_need += 1
                
                if total_need > 0:
                    f.write(f"\t(needs {content_name} {loc} {total_need})\n")
